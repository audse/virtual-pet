...
func _on_drag_started() -> void:
	...
	tween.tween_property(mesh, "mesh:radius", 0.2, 0.15)
	tween.tween_property(mesh, "mesh:height", 0.2, 0.15)
...
func _on_drag_finished() -> void:
	var tween := create_tween().set_ease(Tween.EASE_IN_OUT).set_trans(Tween.TRANS_BACK).set_parallel()
	tween.tween_property(mesh, "mesh:radius", 0.1, 0.25)
	tween.tween_property(mesh, "mesh:height", 0.1, 0.25)
	tween.set_trans(Tween.TRANS_SINE)
